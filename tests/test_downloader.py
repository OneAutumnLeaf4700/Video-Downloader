...
PLAYLIST_URL = "https://www.youtube.com/watch?v=4tmuEf9M9Zs&list=PLSkfZAaBHKPQXzdR7ehNl3KRicesPIIso&ab_channel=ketfut"
...
def cleanup_test_files(output_pattern):
    ...
    # return # Uncomment this out if you wish to inspect the downloaded test files, otherwise keep it commented
    for f in glob.glob(os.path.join(OUTPUT_DIR, output_pattern)):
        os.remove(f)
...
def test_download_with_resolution():
    ...
    setup_test_environment(output_pattern)
    download_video(VIDEO_URL, output_path=output_path, file_format='mp4', resolution='480')

    downloaded_files = glob.glob(os.path.join(OUTPUT_DIR, "test_resolution.mp4"))
    assert len(downloaded_files) > 0, "Video with specific resolution was not downloaded."
    cleanup_test_files(output_pattern)
...
def test_download_playlist():
    ...
    download_video(PLAYLIST_URL, output_path=output_path, is_playlist=True, file_format='mp4')
